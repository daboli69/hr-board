...
def recent_drives(df: pd.DataFrame, start_date: str, min_dist: int = 240, cap: int = 18) -> dict:
    ...
    _has_inn = "inning" in d.columns and "inning_topbot" in d.columns
    out = {}
    for bid, g in d.groupby("batter"):
        ...
        for r in g.itertuples():
            ...
            if _has_inn:
                try:
                    rec["inning"] = int(getattr(r, "inning"))
                except (TypeError, ValueError):
                    pass
                _half = str(getattr(r, "inning_topbot", "") or "")
                if _half:
                    rec["half"] = "T" if _half.lower().startswith("t") else "B"
            recs.append(rec)
        out[int(bid)] = recs
    return out
