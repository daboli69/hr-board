...
(() => {
 ...
 let evidence=null,evidenceBusy=false,evidenceError='';
 function renderEvidence(){
  const games=evidence?.games||[];
  document.getElementById('main').innerHTML=`<section class="going-section"><h2>Pregame research record</h2><p>Only observations saved before first pitch appear here. Older daily snapshots remain separate because they could be overwritten.</p><p>${html(evidence?.note||'This records model scores and actual results. It does not yet measure betting returns or whether predicted chances are accurate.')}</p><p>${evidenceError?html(evidenceError):evidenceBusy?'Checking saved records…':`${games.length} games graded · ${evidence?.pending_games||0} awaiting a final result`}</p><div class="going-grid">${games.slice(-30).reverse().map(g=>`<article class="going-card"><b>${html(g.game?.away)} at ${html(g.game?.home)}</b><span>${html(g.date)} · saved ${html(new Date(g.captured_at).toLocaleString())}</span>${g.players.map(p=>`<p><strong>${html(p.name)}</strong> · score ${html(p.score??'Unavailable')}<br>${p.state==='graded'?`${p.home_runs} home runs · ${p.hits} hits · ${p.hits_runs_rbis} hits + runs + RBIs`:html(p.state.replaceAll('_',' '))}</p>`).join('')}</article>`).join('')||'<p>No completed pregame records yet. New records accumulate as upcoming games finish; past evidence is not reconstructed.</p>'}</div></section>`;
  if(!evidence&&!evidenceBusy&&!evidenceError){evidenceBusy=true;fetch(platform?'/api/yard-snapshot?file=pregame-results.json':'pregame-results.json').then(r=>{if(!r.ok)throw Error();return r.json();}).then(data=>{evidence=data;}).catch(()=>{evidenceError='Pregame records are temporarily unavailable. Try again after the next data update.';}).finally(()=>{evidenceBusy=false;if(view==='pregame-record')renderEvidence();});}
 }
 renderView=function(){document.body.dataset.yardView=view;document.querySelectorAll('[data-going-view]').forEach(b=>b.setAttribute('aria-pressed',String(b.dataset.goingView===view)));if(!BOARD)return originalRender();if(view==='pregame-record')renderEvidence();else if(view==='market-disagreement')renderMarket();else if(view==='fantasy-research')renderFantasy();else originalRender();};
